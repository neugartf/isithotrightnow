...
def lambda_handler(event, context):
    ...
    try:
        ...
        for location in locations:
            distance = (
                float(event["queryStringParameters"]["lat"]) - float(location["lat"])
            ) ** 2 + (
                float(event["queryStringParameters"]["lon"]) - float(location["lon"])
            ) ** 2
...
if __name__ == "__main__":
    ...
    local.serve_handler(lambda_handler, port=8080)
